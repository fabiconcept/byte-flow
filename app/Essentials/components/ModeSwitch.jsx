...
export default function ModeSwitch() {
    ...
    return (
        <div className="dark:bg-dark-themeBackground bg-light-themeBackground w-fit h-fit flex flex-col rounded-3xl relative group my-6" >
            <div title={"Light Mode"} className={`${currentMode === 0 ? "dark:text-black text-dark-textColor" : ""} active:scale-90 active:op cursor-pointer relative z-10 h-8 w-8 rounded-full grid place-items-center`} onClick={()=>setCurrentMode(0)}>
                <CiSun className={`${currentMode === 0 ? "scale-125" : ""} duration-0 text-xl`} />
            </div>
            <div title={"Dark Mode"} className={`${currentMode === 1 ? "dark:text-black text-dark-textColor" : ""} active:scale-90 active:op cursor-pointer relative z-10 h-8 w-8 rounded-full grid place-items-center`} onClick={()=>setCurrentMode(1)}>
