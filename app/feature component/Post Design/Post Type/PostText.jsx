...
export default function PostText({contentHtml, isDemo}) {
    const replacedHtml = replaceCodeTagsWithComponentHtml(contentHtml, <Code />);
    ...
    return (
        <div className="w-full hover:scale-[1.025] rounded-md p-3 dark:bg-dark-themeBackground/90 bg-white/90 backdrop-blur-md flex flex-col gap-3">
            ...
            {isDemo && <section className="text-sm px-2 py-1 line-clamp-6 whitespace-pre-wrap prose dark:prose-invert prose-img:rounded prose-img:max-w-full prose-img:mx-auto prose-img:max-h-96 prose-h3:mb-0 prose-h2:mb-0 prose-h1:mb-0 prose-h3:leading-none prose-h3:mt-0 prose-p:not-prose prose-h2:leading-none prose-h2:mt-0 prose-h1:leading-none prose-h1:mt-0 prose-li:mb-2 prose-a:text-themeMain prose-img:my-0 prose-p:my-0 prose-blockquote:my-0" dangerouslySetInnerHTML={{ __html: replacedHtml ? replacedHtml : "<span>Use markdown to create post</span>"  }} />}
            <PostReactions />
        </div>
    )
}